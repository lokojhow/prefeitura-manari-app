// Prefeitura de Manari — bootstrap V4.2 sem flash do layout legado
(() => {
  if (window.__MANARI_SOCIAL_BOOTSTRAP__) return;
  window.__MANARI_SOCIAL_BOOTSTRAP__=true;
  const VERSION='4.2';
  // Esconde somente o conteúdo legado durante a troca; o fundo permanece estável.
  ...
  const reveal=()=>{document.getElementById('manari-v4-boot-gate')?.remove();document.documentElement.classList.add('manari-v4-ready');};
  ...
  function loadScriptOnce(attr,src,ready,next){if(ready?.()||document.querySelector(`script[${attr}]`)){next?.();return;}const s=document.createElement('script');s.src=`${src}?v=${VERSION}`;s.async=false;s.setAttribute(attr,'true');s.onload=()=>next?.();s.onerror=()=>next?.();document.body.appendChild(s);}
