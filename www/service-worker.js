const CACHE='manari-v4-cms-setorial';
const VERSION='4.0';
...
async function prepareResponse(request,response){
  ...
  if(url.pathname.endsWith('/config.js')){
    ...
    return new Response(loader+original,{status:response.status,statusText:response.statusText,headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-cache'}});
  }
  if(request.mode==='navigate'){
    ...
    if(type.includes('text/html')){
      ...
      const head=[];
      if(!html.includes('social-layout.css')) head.push(`<link rel="stylesheet" href="social-layout.css?v=${VERSION}">`);
      ...
      if(!html.includes('portal-cms.css')) head.push(`<link rel="stylesheet" href="portal-cms.css?v=${VERSION}">`);
      ...
      if(scripts.length){const tags=scripts.join('');html=html.includes('</body>')?html.replace('</body>',`${tags}</body>`):`${html}${tags}`;}
...
self.addEventListener('fetch',e=>{
  ...
  if(e.request.mode==='navigate'){
    e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{const prepared=await prepareResponse(e.request,r);if(prepared&&prepared.ok){const copy=prepared.clone();caches.open(CACHE).then(c=>c.put('index.html',copy));}return prepared;}).catch(async()=>prepareResponse(e.request,await caches.match('index.html'))));
    return;
  }
  e.respondWith(fetch(e.request,{cache:'no-store'}).then(async r=>{const prepared=await prepareResponse(e.request,r);if(prepared&&prepared.ok){const copy=prepared.clone();caches.open(CACHE).then(c=>c.put(e.request,copy));}return prepared;}).catch(()=>caches.match(e.request)));
});